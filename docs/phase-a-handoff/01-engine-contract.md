...
| `required_mentions` | item | `{rules: [{field, must_contain: ["{token}", …], ci?}]}` | substring match AFTER `{token}` resolution from input_data — e.g. `"{price_buy_fmt}"` → `"8.450.000"` must appear in the field. Per-rule `ci: true` matches case-insensitively (NFC + lowercase) — use it when the token is a keyword **phrase** that shifts case across surfaces ("Số chủ đạo 7" title vs "Số Chủ Đạo 7" seoTitle); keep the default for case-exact brand strings |
...
| `keyword_density` | item | `{keywords: ["{searchKeyword}", …], min_count?, max_density?, fields?}` | on-page keyword enforcement over the prose body. Counts each phrase case-insensitively (NFC + lowercase, whitespace collapsed). `count < min_count` (default 1) → under-used; `max_density` set and `count × phraseWords / totalWords` above it → stuffing. `fields` limits the scan (dotted paths like the length gate); default = every string in the item. Default severity **`flag`** — density is reviewer territory, and the phrases come from input_data so the gate stays domain-blind. Empty/missing `keywords` = no-op |
